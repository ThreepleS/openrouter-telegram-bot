...
const twoFaStore = new Map<number, { code: string; expires: number }>();

async function request2FA(adminId: number, action: string): Promise<string> {
  const code = String(Math.floor(100000 + Math.random() * 900000));
  twoFaStore.set(adminId, { code, expires: Date.now() + 300000 });
  await auditLog(getSupabase(true), adminId, "admin_2fa_request", true, action);
  return code;
}

async function verify2FA(adminId: number, payload: any): Promise<boolean> {
  const code = String(payload.confirm || "").trim();
  if (!code) return false;
  const stored = twoFaStore.get(adminId);
  if (!stored || stored.expires < Date.now()) return false;
  if (stored.code !== code) return false;
  twoFaStore.delete(adminId);
  return true;
}
...
Deno.serve(async (req: Request) => {
  ...
  const isDestructive = ["reset", "reset_all"].includes(action);
  ...
  if (action === "request_2fa") {
    const neededFor = String(payload.for_action || "").trim() || "reset";
    const code = await request2FA(adminId!, neededFor);
    return json({ ok: true, code });
  }

  if (isDestructive && !(await verify2FA(adminId!, payload))) {
    await auditLog(supabase, adminId, "admin_2fa_fail", false, action);
    return json({ ok: false, error: "2FA required" }, 403);
  }
